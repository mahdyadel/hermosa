@extends('layouts.app')
@section('title', __('messages.menu.employeeRates') )
        ...
        <div class="tab-content">
            <div class="table-responsive">
                ...
                <table class="table table-striped m-table">
                    ...
                    <!--begin::Tbody-->
                    <tbody>
                        @if(count($employeeRates) > 0)
                            @foreach($employeeRates as $employeeRate)
                            <tr>
                                <td>{{ $employeeRate->id }}</td>
                                <td>{{ $employeeRate->user->name }}</td>
                                <td>{{ $employeeRate->rate }}</td>
                                <td>{{ $employeeRate->comment }}</td>
                                @if(auth()->user()->can('DELETE_ADMINS'))
                                <td>
                                    <form action="/salons/{{ request()->route('salon')}}/employees/{{ request()->route('employee')}}/rates/{{ $employeeRate->id }}" method="POST" style="display: inline;" onsubmit="if(confirm('Delete? Are you sure?')) { return true } else {return false };">
            ...
            <div class="m--align-center">
                {{ $employeeRates->appends($_GET)->links() }}
            </div>
        </div>
    </div>
